...
PACKAGE_DIR = Path(hermes_metrics.__file__).resolve().parent
REPO_ROOT = PACKAGE_DIR.parent
ENTRY_POINT_GROUP = "hermes_agent.plugins"
DISTRIBUTION = "hermes-metrics"
...
def test_manifest_version_matches_the_package() -> None:
    assert str(_manifest()["version"]) == hermes_metrics.__version__
...
def test_entry_point_resolves_to_the_package() -> None:
    ...
    assert entry_points[ENTRY_POINT_GROUP] == {DISTRIBUTION: "hermes_metrics"}


def test_distribution_version_matches_the_package() -> None:
    assert _pyproject()["project"]["version"] == hermes_metrics.__version__  # type: ignore[index]
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    if alias.name.split(".")[0] == "hermes_metrics":
                        offenders.append(f"{source.name}:{node.lineno} import {alias.name}")
            elif isinstance(node, ast.ImportFrom):
                module = node.module or ""
                if node.level == 0 and module.split(".")[0] == "hermes_metrics":
...
def test_the_manifest_declares_every_hook_the_runtime_subscribes_to() -> None:
    from hermes_metrics.runtime import FLUSH_HOOK, HOOK_KINDS
